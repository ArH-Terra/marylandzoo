...
// ID!!!
const int this_node_ID = 4;  // Unique ID for this node

// pin settings
const byte mp3_rxPin = 2;
const byte mp3_txPin = 3;
const byte ultrasonic_rxPin = 4;
const byte ultrasonic_txPin = 5;
...
const int led_G = 10;
const int led_B = 11;
const byte Xbee_rxPin = 12;
const byte Xbee_txPin = 13;
...
// note that compared to the original library, I added "_isSending = 0;" at line 37 in DFRobotDFPlayerMini.cpp
...
// Function ball_release: Lowers linear actuator to release the ball and then raises it back up to original position ready to be loaded again.
void ball_release() {
  digitalWrite(actuator_neg_relay_pin, LOW);
  delay(3000);
  digitalWrite(actuator_neg_relay_pin, HIGH);
  delay(100);
