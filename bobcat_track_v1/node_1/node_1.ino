...
// ultrasonic
unsigned char ultrasonic_readout[4] = {};
float distance;
...
void loop() {
  ...
  if (XBee.available()) {
    ...
    if (receivedMsg.length() >= 14) {
      ...
      audio_volume = receivedMsg.substring(7, 9).toInt();
...
void sendCompletionSignal(unsigned long completionTime) {
  ...
  sprintf(buffer, "%d%03lu", 2000 + this_node_ID, completionTime);
...
void sendTimeoutSignal() {
  String timeoutSignal = String(3000 + this_node_ID);
...
float Detect() {
  do {
    for (int i = 0; i < 4; i++) {
      ultrasonic_readout[i] = ultrasonic_serial.read();
    }
  } while (ultrasonic_serial.read() == 0xff);

  ultrasonic_serial.flush();

  if (ultrasonic_readout[0] == 0xff) {
    int sum;
    sum = (ultrasonic_readout[0] + ultrasonic_readout[1] + ultrasonic_readout[2]) & 0x00FF;
    if (sum == ultrasonic_readout[3]) {
      distance = (ultrasonic_readout[1] << 8) + ultrasonic_readout[2];
      ...
      delay(100);
      return Detect();  // If error in reading ultrasonic_readout, wait 1/10s and take another reading
    }
  }
}
